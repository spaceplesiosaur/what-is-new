  ...
  it('should match the snapshot with correct data', () => {
    ...
    it('should render news articles', () => {
        ...
        const wrapper = shallow( < NewsContainer articleList = {
          ...
          />);
          const expected = ( < NewsArticle description = "Key mitten forest masterpiece raindrop."
            headline = "Music kitchen vase hammock prince dugout" / > );

          expect(wrapper.instance().renderNewsArticles().length).toEqual(3) expect(wrapper.instance().renderNewsArticles()[1].props.headline).toEqual(expected.props.headline)
        })
    });
